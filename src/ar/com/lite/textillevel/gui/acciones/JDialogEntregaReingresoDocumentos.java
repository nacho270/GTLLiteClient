...
import ar.com.lite.textillevel.gui.util.GenericUtils.BackgroundTask;
				...
				GenericUtils.realizarOperacionConDialogoDeEspera(msg, new BackgroundTask() {
					...
					public void perform() {
						try {
							...
							if (modo == Modo.SALIDA) {
								resp = GTLLiteRemoteService.marcarEntregado(txtIngreso.getText());
							} else {
								resp = GTLLiteRemoteService.reingresar(txtIngreso.getText());
							}
							if (resp.isError()) {
